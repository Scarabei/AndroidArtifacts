...
class AppCompatPopupWindow extends PopupWindow {
   ...
   private void init(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
      TintTypedArray a = TintTypedArray.obtainStyledAttributes(context, attrs, styleable.PopupWindow, defStyleAttr, defStyleRes);
      if (a.hasValue(styleable.PopupWindow_overlapAnchor)) {
         this.setSupportOverlapAnchor(a.getBoolean(styleable.PopupWindow_overlapAnchor, false));
      }

      this.setBackgroundDrawable(a.getDrawable(styleable.PopupWindow_android_popupBackground));
      int sdk = VERSION.SDK_INT;
      if (defStyleRes != 0 && sdk < 11 && a.hasValue(styleable.PopupWindow_android_popupAnimationStyle)) {
         this.setAnimationStyle(a.getResourceId(styleable.PopupWindow_android_popupAnimationStyle, -1));
      }
      ...
      try {
         ...
         fieldListener.set(popup, new OnScrollChangedListener() {
            public void onScrollChanged() {
               try {
                  WeakReference mAnchor = (WeakReference)fieldAnchor.get(popup);
         ...
         Log.d("AppCompatPopupWindow", "Exception while installing workaround OnScrollChangedListener", var4);
      }
   ...
   static {
      COMPAT_OVERLAP_ANCHOR = VERSION.SDK_INT < 21;
   }
}
