...
public class AppCompatImageHelper {
   ...
   public void loadFromAttributes(AttributeSet attrs, int defStyleAttr) {
      TintTypedArray a = TintTypedArray.obtainStyledAttributes(this.mView.getContext(), attrs, styleable.AppCompatImageView, defStyleAttr, 0);

      try {
         Drawable drawable = this.mView.getDrawable();
         if (drawable == null) {
            int id = a.getResourceId(styleable.AppCompatImageView_srcCompat, -1);
            if (id != -1) {
         ...
         if (a.hasValue(styleable.AppCompatImageView_tint)) {
            ImageViewCompat.setImageTintList(this.mView, a.getColorStateList(styleable.AppCompatImageView_tint));
         }

         if (a.hasValue(styleable.AppCompatImageView_tintMode)) {
            ImageViewCompat.setImageTintMode(this.mView, DrawableUtils.parseTintMode(a.getInt(styleable.AppCompatImageView_tintMode, -1), (Mode)null));
         }
      } finally {
         a.recycle();
      }
   ...
   boolean hasOverlappingRendering() {
      ...
      return VERSION.SDK_INT < 21 || !(background instanceof RippleDrawable);
   }
