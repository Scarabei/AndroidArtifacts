...
public class SwitchCompat extends CompoundButton {
   ...
   public SwitchCompat(Context context, AttributeSet attrs) {
      this(context, attrs, attr.switchStyle);
   }
   ...
   public SwitchCompat(Context context, AttributeSet attrs, int defStyleAttr) {
      ...
      TintTypedArray a = TintTypedArray.obtainStyledAttributes(context, attrs, styleable.SwitchCompat, defStyleAttr, 0);
      this.mThumbDrawable = a.getDrawable(styleable.SwitchCompat_android_thumb);
      if (this.mThumbDrawable != null) {
         this.mThumbDrawable.setCallback(this);
      }

      this.mTrackDrawable = a.getDrawable(styleable.SwitchCompat_track);
      if (this.mTrackDrawable != null) {
         this.mTrackDrawable.setCallback(this);
      }

      this.mTextOn = a.getText(styleable.SwitchCompat_android_textOn);
      this.mTextOff = a.getText(styleable.SwitchCompat_android_textOff);
      this.mShowText = a.getBoolean(styleable.SwitchCompat_showText, true);
      this.mThumbTextPadding = a.getDimensionPixelSize(styleable.SwitchCompat_thumbTextPadding, 0);
      this.mSwitchMinWidth = a.getDimensionPixelSize(styleable.SwitchCompat_switchMinWidth, 0);
      this.mSwitchPadding = a.getDimensionPixelSize(styleable.SwitchCompat_switchPadding, 0);
      this.mSplitTrack = a.getBoolean(styleable.SwitchCompat_splitTrack, false);
      ColorStateList thumbTintList = a.getColorStateList(styleable.SwitchCompat_thumbTint);
      if (thumbTintList != null) {
         this.mThumbTintList = thumbTintList;
         this.mHasThumbTint = true;
      }

      Mode thumbTintMode = DrawableUtils.parseTintMode(a.getInt(styleable.SwitchCompat_thumbTintMode, -1), (Mode)null);
      ...
      ColorStateList trackTintList = a.getColorStateList(styleable.SwitchCompat_trackTint);
      if (trackTintList != null) {
         this.mTrackTintList = trackTintList;
         this.mHasTrackTint = true;
      }

      Mode trackTintMode = DrawableUtils.parseTintMode(a.getInt(styleable.SwitchCompat_trackTintMode, -1), (Mode)null);
      ...
      int appearance = a.getResourceId(styleable.SwitchCompat_switchTextAppearance, 0);
      if (appearance != 0) {
   ...
   public void setSwitchTextAppearance(Context context, int resid) {
      TintTypedArray appearance = TintTypedArray.obtainStyledAttributes(context, resid, styleable.TextAppearance);
      ColorStateList colors = appearance.getColorStateList(styleable.TextAppearance_android_textColor);
      if (colors != null) {
      ...
      int ts = appearance.getDimensionPixelSize(styleable.TextAppearance_android_textSize, 0);
      if (ts != 0 && (float)ts != this.mTextPaint.getTextSize()) {
         this.mTextPaint.setTextSize((float)ts);
         this.requestLayout();
      }

      int typefaceIndex = appearance.getInt(styleable.TextAppearance_android_typeface, -1);
      int styleIndex = appearance.getInt(styleable.TextAppearance_android_textStyle, -1);
      this.setSwitchTypefaceByIndex(typefaceIndex, styleIndex);
      boolean allCaps = appearance.getBoolean(styleable.TextAppearance_textAllCaps, false);
      if (allCaps) {
   ...
   public void onPopulateAccessibilityEvent(AccessibilityEvent event) {
      ...
      CharSequence text = this.isChecked() ? this.mTextOn : this.mTextOff;
      if (text != null) {
   ...
   public void draw(Canvas c) {
      ...
      if (this.mTrackDrawable != null) {
         ...
         thumbRight = switchTop;
         int trackRight = switchRight;
         int trackBottom = switchBottom;
         if (thumbInsets != null) {
            if (thumbInsets.left > padding.left) {
               thumbLeft = switchLeft + (thumbInsets.left - padding.left);
            }

            if (thumbInsets.top > padding.top) {
               thumbRight = switchTop + (thumbInsets.top - padding.top);
            }
         ...
         this.mTrackDrawable.setBounds(thumbLeft, thumbRight, trackRight, trackBottom);
      }

      if (this.mThumbDrawable != null) {
         this.mThumbDrawable.getPadding(padding);
         thumbLeft = thumbInitialLeft - padding.left;
         thumbRight = thumbInitialLeft + this.mThumbWidth + padding.right;
   ...
   public void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfo info) {
      if (VERSION.SDK_INT >= 14) {
         ...
         CharSequence switchText = this.isChecked() ? this.mTextOn : this.mTextOff;
