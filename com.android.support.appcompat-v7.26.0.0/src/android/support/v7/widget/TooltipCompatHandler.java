...
class TooltipCompatHandler implements OnLongClickListener, OnHoverListener, OnAttachStateChangeListener {
   ...
   public static void setTooltipText(View view, CharSequence tooltipText) {
      if (TextUtils.isEmpty(tooltipText)) {
         ...
         view.setOnLongClickListener((OnLongClickListener)null);
         view.setLongClickable(false);
         view.setOnHoverListener((OnHoverListener)null);
      } else {
   ...
   private void show(boolean fromTouch) {
      if (ViewCompat.isAttachedToWindow(this.mAnchor)) {
         ...
         if (this.mFromTouch) {
            ...
            timeout = 3000L - (long)ViewConfiguration.getLongPressTimeout();
         } else {
            timeout = 15000L - (long)ViewConfiguration.getLongPressTimeout();
         }
