...
class MediaControllerCompatApi21 {
   ...
   public static Object createCallback(MediaControllerCompatApi21.Callback callback) {
      return new MediaControllerCompatApi21.CallbackProxy(callback);
   }
   ...
   public static List getQueue(Object controllerObj) {
      List queue = ((MediaController)controllerObj).getQueue();
      if (queue == null) {
         return null;
      } else {
         List queueObjs = new ArrayList(queue);
         return queueObjs;
      }
   }
   ...
   static class CallbackProxy extends android.media.session.MediaController.Callback {
      ...
      public CallbackProxy(MediaControllerCompatApi21.Callback callback) {
      ...
      public void onAudioInfoChanged(android.media.session.MediaController.PlaybackInfo info) {
         this.mCallback.onAudioInfoChanged(info.getPlaybackType(), MediaControllerCompatApi21.PlaybackInfo.getLegacyAudioStream(info), info.getVolumeControl(), info.getMaxVolume(), info.getCurrentVolume());
      }
   }
