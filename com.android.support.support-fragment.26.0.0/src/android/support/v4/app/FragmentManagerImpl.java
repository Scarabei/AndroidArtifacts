...
final class FragmentManagerImpl extends FragmentManager implements Factory2 {
   ...
   static boolean modifiesAlpha(Animator anim) {
      if (anim == null) {
         ...
         if (anim instanceof ValueAnimator) {
            ...
            List animList = ((AnimatorSet)anim).getChildAnimations();
      ...
      synchronized(this) {
         if (this.mBackStackIndices != null) {
            ...
            if (N > 0) {
               ...
               for(int i = 0; i < N; ++i) {
                  BackStackRecord bs = (BackStackRecord)this.mBackStackIndices.get(i);
                  writer.print(prefix);
                  writer.print("  #");
                  writer.print(i);
                  writer.print(": ");
   ...
   static FragmentManagerImpl.AnimationOrAnimator makeOpenCloseAnimation(Context context, float startScale, float endScale, float startAlpha, float endAlpha) {
      ...
      return new FragmentManagerImpl.AnimationOrAnimator(set, null);
   }
   ...
   static FragmentManagerImpl.AnimationOrAnimator makeFadeAnimation(Context context, float start, float end) {
      ...
      return new FragmentManagerImpl.AnimationOrAnimator(anim, null);
   }
   ...
   FragmentManagerImpl.AnimationOrAnimator loadAnimation(Fragment fragment, int transit, boolean enter, int transitionStyle) {
      ...
      if (animation != null) {
         return new FragmentManagerImpl.AnimationOrAnimator(animation, null);
      } else {
         Animator animator = fragment.onCreateAnimator(transit, enter, nextAnim);
         if (animator != null) {
            return new FragmentManagerImpl.AnimationOrAnimator(animator, null);
         } else {
            if (nextAnim != 0) {
   ...
   private void executePostponedTransaction(ArrayList records, ArrayList isRecordPop) {
      ...
      for(int i = 0; i < numPostponed; ++i) {
         ...
         if (listener.isReady() || records != null && listener.mRecord.interactsWith(records, 0, records.size())) {
            ...
            --i;
            --numPostponed;
            if (records != null && !listener.mIsBack && (index = records.indexOf(listener.mRecord)) != -1 && ((Boolean)isRecordPop.get(index)).booleanValue()) {
               listener.cancelTransaction();
            } else {
   ...
   private void executeOpsTogether(ArrayList records, ArrayList isRecordPop, int startIndex, int endIndex) {
      ...
      for(postponeIndex = startIndex; postponeIndex < endIndex; ++postponeIndex) {
         ...
         boolean isPop = ((Boolean)isRecordPop.get(postponeIndex)).booleanValue();
         if (!isPop) {
   ...
   private int postponePostponableTransactions(ArrayList records, ArrayList isRecordPop, int startIndex, int endIndex, ArraySet added) {
   ...
   boolean popBackStackState(ArrayList records, ArrayList isRecordPop, String name, int id, int flags) {
      if (this.mBackStack == null) {
         return false;
      } else {
         int index;
   ...
   private static void setRetaining(FragmentManagerNonConfig nonConfig) {
      if (nonConfig != null) {
         ...
         List children = nonConfig.getChildNonConfigs();
         if (children != null) {
         ...
         if (fms.mActive != null) {
            ...
            if (fms.mAdded != null) {
               for(i = 0; i < fms.mAdded.length; ++i) {
                  Fragment f = (Fragment)this.mActive.get(fms.mAdded[i]);
                  if (f == null) {
   ...
   public void registerFragmentLifecycleCallbacks(FragmentManager.FragmentLifecycleCallbacks cb, boolean recursive) {
      this.mLifecycleCallbacks.add(new Pair(cb, recursive));
   }
   ...
   void dispatchOnFragmentPreAttached(Fragment f, Context context, boolean onlyRecursive) {
      ...
      while(true) {
         ...
         do {
            ...
            p = (Pair)var6.next();
         } while(onlyRecursive && !((Boolean)p.second).booleanValue());
   ...
   private static class AnimationListenerWrapper implements AnimationListener {
      ...
      // $FF: synthetic method
      AnimationListenerWrapper(AnimationListener x0, Object x1) {
         this(x0);
      }
   }
   ...
   private static class AnimationOrAnimator {
      ...
      // $FF: synthetic method
      AnimationOrAnimator(Animation x0, Object x1) {
         this(x0);
      }

      // $FF: synthetic method
      AnimationOrAnimator(Animator x0, Object x1) {
         this(x0);
      }
   }
