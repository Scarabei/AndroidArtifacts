...
public class FrameMetricsAggregator {
   ...
   public FrameMetricsAggregator(int metricTypeFlags) {
      if (VERSION.SDK_INT >= 24) {
         this.mInstance = new FrameMetricsAggregator.FrameMetricsApi24Impl(metricTypeFlags);
      } else {
   ...
   private static class FrameMetricsApi24Impl extends FrameMetricsAggregator.FrameMetricsBaseImpl {
      ...
      void addDurationItem(SparseIntArray buckets, long duration) {
         if (buckets != null) {
            int durationMs = (int)((duration + 500000L) / 1000000L);
            if (duration >= 0L) {
      ...
      public SparseIntArray[] remove(Activity activity) {
         ...
         while(var2.hasNext()) {
            WeakReference activityRef = (WeakReference)var2.next();
      ...
      public SparseIntArray[] stop() {
         ...
         for(int i = size - 1; i >= 0; --i) {
            WeakReference ref = (WeakReference)this.mActivities.get(i);
   ...
   private static class FrameMetricsBaseImpl {
      ...
      // $FF: synthetic method
      FrameMetricsBaseImpl(Object x0) {
         this();
      }
   }
