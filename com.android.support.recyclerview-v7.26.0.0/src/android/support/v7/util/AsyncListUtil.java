...
public class AsyncListUtil {
   ...
   public AsyncListUtil(Class klass, int tileSize, AsyncListUtil.DataCallback dataCallback, AsyncListUtil.ViewCallback viewCallback) {
      ...
      this.mMainThreadCallback = new ThreadUtil.MainThreadCallback() {
         ...
         public void addTile(int generation, TileList.Tile tile) {
            if (!this.isRequestedGeneration(generation)) {
               ...
               TileList.Tile duplicate = AsyncListUtil.this.mTileList.addOrReplace(tile);
               if (duplicate != null) {
         ...
         public void removeTile(int generation, int position) {
            if (this.isRequestedGeneration(generation)) {
               TileList.Tile tile = AsyncListUtil.this.mTileList.removeAtPos(position);
               if (tile == null) {
      ...
      this.mBackgroundCallback = new ThreadUtil.BackgroundCallback() {
         ...
         private TileList.Tile acquireTile() {
            if (this.mRecycledRoot != null) {
               TileList.Tile result = this.mRecycledRoot;
   ...
   public Object getItem(int position) {
      if (position >= 0 && position < this.mItemCount) {
         Object item = this.mTileList.getItemAt(position);
   ...
   public abstract static class DataCallback {
      ...
      @WorkerThread
      public void recycleData(Object[] data, int itemCount) {
      }
