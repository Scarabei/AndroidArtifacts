...
public class StaggeredGridLayoutManager extends RecyclerView.LayoutManager implements RecyclerView.SmoothScroller.ScrollVectorProvider {
   ...
   public StaggeredGridLayoutManager(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
      ...
      this.setAutoMeasureEnabled(this.mGapStrategy != 0);
   ...
   void updateAnchorInfoForLayout(RecyclerView.State state, StaggeredGridLayoutManager.AnchorInfo anchorInfo) {
   ...
   private boolean updateAnchorFromChildren(RecyclerView.State state, StaggeredGridLayoutManager.AnchorInfo anchorInfo) {
   ...
   boolean updateAnchorFromPendingData(RecyclerView.State state, StaggeredGridLayoutManager.AnchorInfo anchorInfo) {
   ...
   private void measureChildWithDecorationsAndMargin(View child, StaggeredGridLayoutManager.LayoutParams lp, boolean alreadyMeasured) {
      if (lp.mFullSpan) {
         if (this.mOrientation == 1) {
            this.measureChildWithDecorationsAndMargin(child, this.mFullSizeSpec, getChildMeasureSpec(this.getHeight(), this.getHeightMode(), 0, lp.height, true), alreadyMeasured);
         } else {
            this.measureChildWithDecorationsAndMargin(child, getChildMeasureSpec(this.getWidth(), this.getWidthMode(), 0, lp.width, true), this.mFullSizeSpec, alreadyMeasured);
         }
      } else if (this.mOrientation == 1) {
         this.measureChildWithDecorationsAndMargin(child, getChildMeasureSpec(this.mSizePerSpan, this.getWidthMode(), 0, lp.width, false), getChildMeasureSpec(this.getHeight(), this.getHeightMode(), 0, lp.height, true), alreadyMeasured);
      } else {
         this.measureChildWithDecorationsAndMargin(child, getChildMeasureSpec(this.getWidth(), this.getWidthMode(), 0, lp.width, true), getChildMeasureSpec(this.mSizePerSpan, this.getHeightMode(), 0, lp.height, false), alreadyMeasured);
      }
   ...
   private int updateSpecWithExtra(int spec, int startInset, int endInset) {
      if (startInset == 0 && endInset == 0) {
         ...
         return mode != Integer.MIN_VALUE && mode != 1073741824 ? spec : MeasureSpec.makeMeasureSpec(Math.max(0, MeasureSpec.getSize(spec) - startInset - endInset), mode);
      }
   }
   ...
   private void fixEndGap(RecyclerView.Recycler recycler, RecyclerView.State state, boolean canOffsetChildren) {
      int maxEndLine = this.getMaxEnd(Integer.MIN_VALUE);
   ...
   private void fixStartGap(RecyclerView.Recycler recycler, RecyclerView.State state, boolean canOffsetChildren) {
      int minStartLine = this.getMinStart(Integer.MAX_VALUE);
      ...
      for(added = false; layoutState.hasMore(state) && (this.mLayoutState.mInfinite || !this.mRemainingSpans.isEmpty()); added = true) {
         ...
         if (lp.mFullSpan && layoutState.mItemDirection == -1) {
            if (assignSpan) {
               ...
               if (hasInvalidGap) {
                  StaggeredGridLayoutManager.LazySpanLookup.FullSpanItem fullSpanItem = this.mLazySpanLookup.getFullSpanItem(position);
   ...
   class Span {
      ...
      void cacheReferenceLineAndClear(boolean reverseLayout, int offset) {
         int reference;
         if (reverseLayout) {
            reference = this.getEndLine(Integer.MIN_VALUE);
         } else {
            reference = this.getStartLine(Integer.MIN_VALUE);
         }
